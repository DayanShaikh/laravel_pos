                    ...
                    <div class="card my-4">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                <h6 class="text-white mx-3">Manage Account</h6>
                            </div>
                        </div>
                        ...
                        <div class=" me-3 my-3 text-end">
                            <a class="btn bg-gradient-dark mb-0" href="{{ route('account.create')}}"><i class="material-icons text-sm">add</i></a>
                        </div>
                        <form method="GET" action="{{ route('account_bulkAction') }}" id="myForm">
                            @csrf
                            @method('POST')
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td class="align-middle text-center">
                                                    <div class="form-check check-tables">
                                                        <input class="form-check-input" name="multidelete[]" type="checkbox" value="{{$accounts->id}}">
                                                    </div>
                                                </td>
                                                <td class="align-middle text-center">
                                                ...
                                                <td class="align-middle text-center">
                                                    <span class="text-secondary text-sm">{{ $accounts->title}}</span>
                                                </td>
                                                <td class="align-middle text-center">
                                                    <span class="text-secondary text-sm">@if ($accounts->type==1)
                                                        Current Asset
                                                        @elseif ($accounts->type==2)
                                                        Fixed Asset 
                                                        @elseif ($accounts->type==3)
                                                        Capital                                                        
                                                    @endif</span>
                                                </td>
                                                <td class="align-middle text-center">
                                                    <span class="text-secondary text-sm">{{ $accounts->payment}}</span>
                                                </td>
                                                <td class="align-middle text-center">
                                                    <span class="text-secondary text-sm">{{ $accounts->description}}</span>
                                                </td>
                                                <td class="align-middle text-end px-4">
                                                    {{-- <a href="{{ route('customer.ledger', $customers->id) }}" class="btn text-success btn-link pbtn fs-6 p-2" title="Ledger">
                                                        <i class="fa fa-print"></i>
                                                    </a> --}}
                                                    <a rel="tooltip" class="btn text-success btn-link pbtn fs-6 p-2" href="{{ route('account.edit', $accounts->id)}}" title="Edit">
                                                        <i class="material-icons">edit</i>
                                                        <div class="ripple-container"></div>
                                                    </a>
                                                    @if($accounts->status == 0)
                                                    <a href=" {{ route('account_status', [$accounts->id, 1]) }}" class="btn text-danger btn-link pbtn fs-6 p-2" title="Status OFF">
                                                        <i class="fa fa-eye-slash"></i>
                                                    </a>
                                                    @elseif($accounts->status == 1)
                                                    <a href="{{ route('account_status', [$accounts->id, 0]) }}" class="btn text-success btn-link pbtn fs-6 p-2" title="Status On">
                                                        <i class="fa fa-eye"></i>
                                                    </a>
                                                    @endif
                                                    <a href="javascript:void(0)" id="delete-user" data-url="{{ route('account.destroy', $accounts->id) }}" class="btn text-danger btn-link pbtn fs-6 p-2" title="delete">
                            ...
                            <div class="me-5 text-start ml-260 ">
                                <div class="input-group input-group-outline is-filled form-select d-inline-flex w-40 float-start ">
                                    ...
                                    <select onchange="window.location.href=this.value" class="form-control">
                                        @for ($i = 1; $i <= $account->lastPage(); $i++)
                                            <option value="{{ $account->url($i) }}" {{ $account->currentPage() == $i ? 'selected' : '' }}>
                                                {{ $i }}
                                            </option>
                                            @endfor
                                    </select>
