                ...
                <div class="col-12 ">
                    <div class="card z-index-0 fadeIn3 fadeInBottom">
                            ...
                            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-3">
                                <div class="row">
                                    <div class="col my-xl-2">
                                        <h6 class="text-white text-capitalize ps-3">Update Expense</h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body p-0 px-3">
                            <form role="form" method="POST" action="{{  route('manage_expense.update',$detail->id) }}" class="text-start">
                                @csrf
                                @method("PUT")
                                <div class="input-group input-group-outline mt-3 form-select">
                                    <select name="expense_id" id="action" class="form-control">
                                        ...
                                        <option value="{{$expenses->id}}" @if ($expenses->id==$detail->expense_id) selected @endif>{{$expenses->title}}</option>
                                        @endforeach
                                    </select>
                                    @error('expense_id')
                                    ...
                                    <div class="input-group input-group-outline mt-3 null is-filled ">
                                        ...
                                        <input type="text" id="datepicker" class="form-control" name="date" value="{{ $detail->date }}">
                                    </div>
                                        @error('date')
                                        <p class='text-danger inputerror'>{{ $message }} </p>
                                        @enderror
